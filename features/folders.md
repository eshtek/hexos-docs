...
Only those with network credentials created in HexOS can access these folders.<br> This means by default only the administrator has access. The administrator grants access to anyone given network credentials. Folder permissions for this type of folder can be modified.
...
File encryption is when a file's data is scrambled in a way it can only be recovered with a specific encryption key or passphrase.<br> When we create an encrypted folder, all of the contents placed in that folder will also be encrypted. The server administrator can lock (encrypt) or unlock (decrypt) folders from the folder section of HexOS deck.<br> When a folder is locked, the folder and its contents are not visible nor accessible to anyone on the network. When a folder is unlocked it will work the same as a regular folder.<br>
