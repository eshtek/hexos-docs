# Apps (Curated Scripts)

<!-- curated:index:start -->
| App | Download | Size | Last Modified |
|---|---|---:|---|
| `bazarr` | [bazarr.json](/install-scripts/bazarr.json) | 1.0 KB | 2025-12-23 |
| `drawio` | [drawio.json](/install-scripts/drawio.json) | 236 B | 2025-12-23 |
| `emby` | [emby.json](/install-scripts/emby.json) | 2.2 KB | 2025-12-23 |
| `handbrake` | [handbrake.json](/install-scripts/handbrake.json) | 2.6 KB | 2025-12-23 |
| `home-assistant` | [home-assistant.json](/install-scripts/home-assistant.json) | 1.6 KB | 2025-12-04 |
| `immich` | [immich.json](/install-scripts/immich.json) | 1.7 KB | 2025-12-12 |
| `jellyfin` | [jellyfin.json](/install-scripts/jellyfin.json) | 2.2 KB | 2025-12-12 |
| `lidarr` | [lidarr.json](/install-scripts/lidarr.json) | 1.1 KB | 2025-12-23 |
| `nextcloud` | [nextcloud.json](/install-scripts/nextcloud.json) | 3.2 KB | 2025-12-23 |
| `peanut` | [peanut.json](/install-scripts/peanut.json) | 481 B | 2025-12-23 |
| `plex` | [plex.json](/install-scripts/plex.json) | 3.4 KB | 2025-12-12 |
| `prowlarr` | [prowlarr.json](/install-scripts/prowlarr.json) | 766 B | 2025-12-04 |
| `qbittorrent` | [qbittorrent.json](/install-scripts/qbittorrent.json) | 1.0 KB | 2025-12-04 |
| `radarr` | [radarr.json](/install-scripts/radarr.json) | 1.2 KB | 2025-12-04 |
| `scrutiny` | [scrutiny.json](/install-scripts/scrutiny.json) | 1.0 KB | 2025-12-23 |
| `sonarr` | [sonarr.json](/install-scripts/sonarr.json) | 1.2 KB | 2025-12-04 |
| `syncthing` | [syncthing.json](/install-scripts/syncthing.json) | 1.7 KB | 2025-12-23 |
<!-- curated:index:end -->
